...
class Invoice(models.Model):
    ...
    # ── CNAM split ────────────────────────────────────────────────────────────
    # For patients on a CNAM scheme, total_amount = patient_share + cnam_share.
    #
    # • patient_share  : amount collected at the counter (ticket modérateur).
    # • cnam_share     : portion settled by CNAM directly with the clinic.
    #
    # For patients with no CNAM coverage both fields stay at 0.00 and
    # balance_due simply equals total_amount minus payments received.
    #
    # is_apci flags Affections Prises en Charge Intégralement (24 long-term
    # illnesses recognised by CNAM). When True: patient_share = 0, no annual
    # cap applies, and the CNAM covers 100 % of the act.
    ...
    cnam_share = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=Decimal("0.00"),
        help_text="Part prise en charge directement par la CNAM.",
    )
    is_apci = models.BooleanField(
        default=False,
        help_text=(
    ...
    def balance_due(self):
        """Amount still owed by the patient at the counter.

        When a CNAM split has been set, only patient_share is collected from
        the patient — the cnam_share is settled separately by CNAM. When no
        CNAM split is set (both shares are 0), the full total_amount is owed.
        """
        if self.patient_share > Decimal("0.00") or self.cnam_share > Decimal("0.00"):
            # CNAM invoice: patient owes only their share.
            balance = Decimal(str(self.patient_share)) - self.paid_amount
        else:
            # Standard invoice: patient owes the full amount.
    ...
    def refresh_payment_status(self):
        ...
        # When a CNAM split exists, the invoice is fully settled from the
        # patient's side once their share has been paid — CNAM's portion is
        # tracked through CNAMClaim, not through payments on this invoice.
        if self.patient_share > Decimal("0.00") or self.cnam_share > Decimal("0.00"):
            amount_to_clear = Decimal(str(self.patient_share))
        else:
            amount_to_clear = Decimal(str(self.total_amount))

        next_status = self.Status.PAID if self.paid_amount >= amount_to_clear else self.Status.ISSUED
    ...
    def recalculate_totals(self):
        ...
        # patient_share and cnam_share are set explicitly by the caller
        # (e.g. from the API when creating a CNAM invoice). We validate that
        # the split is coherent: the two portions must not exceed total_amount.
        # If they do (e.g. line items were removed after the split was set),
        # reset both to zero so the invoice falls back to a standard invoice.
        if self.patient_share + self.cnam_share > self.total_amount:
            self.patient_share = Decimal("0.00")
            self.cnam_share = Decimal("0.00")

        self.save(update_fields=[
            "subtotal", "tax_amount", "total_amount",
            "patient_share", "cnam_share",
            "updated_at",
        ])
