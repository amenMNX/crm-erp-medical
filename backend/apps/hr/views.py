...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="check-employee-number",
        permission_classes=[HrPermission],
    )
    def check_employee_number(self, request):
        """
        GET /api/hr/employees/check-employee-number/?value=EMP-0010

        Returns { "available": true } if the employee_number is free,
        or { "available": false, "conflict_id": 3, "conflict_name": "Ali Mansouri" }
        if it is already taken (excluding the employee being edited, if any).
        """
        value = (request.query_params.get("value") or "").strip()
        if not value:
            return Response({"available": False, "detail": "No value provided."}, status=status.HTTP_400_BAD_REQUEST)

        # When editing, exclude the current employee so the existing number
        # doesn't flag itself as a conflict.
        ...
        if conflict:
            return Response({
                ...
                "conflict_name": f"{conflict.first_name} {conflict.last_name}".strip(),
            })
