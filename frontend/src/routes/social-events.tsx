...
import { Calendar, Gift, Loader2, MapPin, Plus, UserMinus, UserPlus, Users } from "lucide-react";
...
import { createSocialEvent, fetchSocialEvents, fetchUpcomingBirthdays, registerForEvent, unregisterFromEvent, type ApiSocialEvent, type EventType, type SocialEventPayload } from "@/lib/social-events-api";
...
function SocialEventsPage() {
  ...
  const birthdaysQuery = useQuery({ queryKey: ["birthdays-upcoming"], queryFn: fetchUpcomingBirthdays });
  ...
  const createMutation   = useMutation({ mutationFn: createSocialEvent,     onSuccess: () => { invalidate(); setOpen(false); toast.success("Événement créé."); }, onError: () => toast.error("Erreur.") });
  ...
  const EventList = ({ list }: { list: ApiSocialEvent[] }) => (
    <div className="space-y-3">
          ...
          <Card key={ev.id}>
            <CardHeader className="flex flex-row items-start justify-between pb-2">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs text-muted-foreground">{ev.participant_count}{ev.max_participants > 0 ? `/${ev.max_participants}` : ""}</span>
                {ev.status === "planned" && (
                  <Button size="sm" variant="outline" disabled={registerMutation.isPending || (ev.is_full && !registered)} onClick={() => registerMutation.mutate({ id: ev.id, registered })}>
                    {registered ? <><UserMinus className="h-3 w-3 mr-1" />Se désinscrire</> : <><UserPlus className="h-3 w-3 mr-1" />S'inscrire</>}
                  </Button>
                )}
      ...
      {list.length === 0 && <p className="text-center py-8 text-sm text-muted-foreground">Aucun événement.</p>}
    </div>
  );
  ...
  return (
    ...
    >
      <Tabs defaultValue="upcoming">
        <TabsList>
          <TabsTrigger value="upcoming"><Calendar className="h-4 w-4 mr-1" />À venir ({upcoming.length})</TabsTrigger>
          <TabsTrigger value="all">Tous ({events.length})</TabsTrigger>
          <TabsTrigger value="birthdays"><Gift className="h-4 w-4 mr-1" />Anniversaires ({birthdays.length})</TabsTrigger>
        </TabsList>
        <TabsContent value="upcoming" className="mt-3">{eventsQuery.isLoading ? <div className="flex items-center gap-2 py-8 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Chargement...</div> : <EventList list={upcoming} />}</TabsContent>
        <TabsContent value="all" className="mt-3">{eventsQuery.isLoading ? <div className="flex items-center gap-2 py-8 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Chargement...</div> : <EventList list={events} />}</TabsContent>
        <TabsContent value="birthdays" className="space-y-3 mt-3">
          {birthdaysQuery.isLoading && <div className="flex items-center gap-2 py-8 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Chargement...</div>}
          {birthdays.map((b) => (
            <Card key={b.id}>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div><CardTitle className="text-base">{b.employee_name}</CardTitle><Badge className="bg-pink-100 text-pink-700 border-0 mt-1">🎂 {b.age} ans</Badge></div>
                <div className="text-right text-sm"><p className="text-muted-foreground">Prochain anniversaire</p><p className="font-medium">{b.next_birthday}</p></div>
              </CardHeader>
            </Card>
          ))}
          {!birthdaysQuery.isLoading && birthdays.length === 0 && <p className="text-center py-8 text-sm text-muted-foreground">Aucun anniversaire dans les 30 prochains jours.</p>}
        </TabsContent>
      </Tabs>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {detail && (
            <div className="space-y-3 text-sm">
              <div className="flex gap-2 flex-wrap"><Badge className={ec(detail.event_type)}>{detail.event_type_display}</Badge><Badge className={sc(detail.status)}>{detail.status_display}</Badge></div>
              ...
              {detail.description && <p className="text-muted-foreground whitespace-pre-wrap">{detail.description}</p>}
              {detail.participants_list.length > 0 && (
                <div>
                  <p className="font-medium mb-2">Participants</p>
                  <div className="flex flex-wrap gap-1">{detail.participants_list.map((n, i) => <Badge key={i} variant="outline" className="text-xs">{n}</Badge>)}</div>
                </div>
              )}
            </div>
          )}
